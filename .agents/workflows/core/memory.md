...
Save session insights to `.agents/storage/` for future reference (directory is auto-created on first use).
...
3. **Saves** to `.agents/storage/memory-{YYYY-MM-DD}.md`
...
Creates: `.agents/storage/memory-{YYYY-MM-DD}.md`
...
3. Write to storage:
   - Create .agents/storage/ if not exists
...
Saved to: .agents/storage/memory-2026-01-23.md
