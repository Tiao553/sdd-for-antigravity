...
**Architect agents** (8 agents in `.agents/agents/architect/`):
...
**Cloud agents** (10 agents in `.agents/agents/cloud/`):
...
**Platform agents** (6 agents in `.agents/agents/platform/`):
...
**Data engineering agents** (15 agents in `.agents/agents/data-engineering/`):
...
**Python agents** (6 agents in `.agents/agents/python/`):
...
**Test agents** (3 agents in `.agents/agents/test/`):
...
```bash
mkdir -p .agents/kb/{domain}/{concepts,patterns}
```

2. Copy templates from `_templates/`:

```bash
cp .agents/kb/_templates/index.md.template .agents/kb/{domain}/index.md
cp .agents/kb/_templates/quick-reference.md.template .agents/kb/{domain}/quick-reference.md
cp .agents/kb/_templates/concept.md.template .agents/kb/{domain}/concepts/{name}.md
cp .agents/kb/_templates/pattern.md.template .agents/kb/{domain}/patterns/{name}.md
```
...
The machine-readable registry lives at `.agents/kb/_index.yaml`. It defines:
...
- **domains** -- Complete registry of all 23 domains with:
  ...
  - `path` -- Directory path relative to `.agents/kb/`
