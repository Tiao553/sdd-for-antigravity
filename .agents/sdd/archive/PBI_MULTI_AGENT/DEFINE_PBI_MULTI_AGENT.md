...
| **MUST** | Integrar com Shadow Traffic via Docker para geração de dados sintéticos de alta fidelidade em banco de dados Supabase/Postgres |
...
- **Conexão a bancos legados/on-premise** — foco inicial em Supabase/Postgres via Cloud ou Docker local
...
| **Data Generation** | [Shadow Traffic](https://shadowtraffic.io/) | Geração de dados sintéticos baseada em schema, alta fidelidade |
| **Database** | Supabase (Postgres) | Storage para os dados gerados, leitura de schema para os agentes |
| **Infrastructure** | Docker Compose | Orquestração do Shadow Traffic e database local para dev |
