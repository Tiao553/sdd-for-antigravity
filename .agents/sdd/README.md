...
| **Target Audience** | Data engineering teams using Antigravity |
| **License** | MIT |
...
Domains are registered in `.agents/kb/_index.yaml`. Use `/create-kb` to add new domains.
...
| `/sync-context` | Update GEMINI.md from codebase |
...
|----|------------|------|--------|
| C-001 | Must work with Antigravity CLI | Technical | Required |
...
```bash
mkdir -p .agents/kb/iceberg/{concepts,patterns}
touch .agents/kb/iceberg/{index.md,quick-reference.md}
```

3. **Register in KB index** (`.agents/kb/_index.yaml`):

```yaml
domains:
  iceberg:
    description: "Apache Iceberg table format patterns"
    entry_point: ".agents/kb/iceberg/index.md"
```
...
/define .agents/sdd/features/BRAINSTORM_ORDERS_PIPELINE.md

# Phase 2: Design with Agent Matching
/design .agents/sdd/features/DEFINE_ORDERS_PIPELINE.md

# Phase 3: Build with Agent Delegation
/build .agents/sdd/features/DESIGN_ORDERS_PIPELINE.md

# Phase 4: Archive
/ship .agents/sdd/features/DEFINE_ORDERS_PIPELINE.md
```
...
| Resource | Location |
|----------|----------|
| SDD Index | `.agents/sdd/_index.md` |
| Architecture | `.agents/sdd/architecture/ARCHITECTURE.md` |
